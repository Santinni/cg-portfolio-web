...
## 2026-09-11 — Orchestrated completion run (COD-78–85)

- **Scope:** seven Linear issues delivered as PRs #51–#57 by a Claude Code controller delegating to Codex CLI, GitHub Copilot CLI, Mistral Vibe and native Claude subagents, each in its own git worktree.
- **Incident — Codex quota:** both Codex packets (COD-78, COD-85) died at 00:37 with "You've hit your usage limit … try again at 3:01 AM" before the first edit, after reading `AGENTS.md` and four skill files. No repository change resulted. Corrective action: rerouted to Claude subagents rather than a pricier model; check Codex quota timing before assigning it anything time-critical.
- **Incident — worker sandboxes:** the sandbox classifier refused copying `.env` into worktrees (secrets), so external workers ran only unit tests, typecheck, lint and format; `pnpm build` and Playwright ran in the pinned container under the controller. Two probe specs (WebKit asset loading, filter navigation) were run through `PLAYWRIGHT_ARGS` in that container instead of on a local WebKit, whose installed build (2215) did not match the Playwright 1.62 requirement (2336); installing a browser without an explicit go was not done.
- **Incident — controller tooling:** a single Bash call bundling several long quoted heredocs (packets, PR bodies) failed to parse and executed nothing, twice. Corrective action: long text goes to a scratch file through the Write tool and is referenced (`git commit -F`, a Node script for the GitHub REST calls; `gh` is not installed and Python lacked CA certificates).
- **Independent review:** Vibe (read-only, stdin packet, no tools) reviewed every diff; its hit-area finding on COD-80 and its build-time env-var finding on the COD-85 header change were accepted and fixed, its locale-prefix and 404-locale concerns were refuted by running the tests. Copilot's test-gap review of COD-80 led to a server-query assertion and a font-settle wait.
- **Verification cost:** six pinned container runs (30.8 min for the four-engine matrix, 2–4 min each for confirmations) plus CI on every PR. The container serialises everything, so the runs were queued rather than parallelised.
- **Outcome:** all seven PRs green in CI at the time of writing; merges remain with the owner.

## 2026-09-13 — Review council for the Národní knihovna CV entry (PR #63)

- **Scope:** controller (Claude Code) wrote the new `nkp` CV entry, the seventh Worked-with mark and the pinned re-measurements; Mistral Vibe reviewed the EN/CS copy (stdin packet, no tools, one turn); GitHub Copilot CLI ran a read-only test-gap review (`--mode plan`, `git diff`, file reads); a native Explore subagent summarised the client's project folder and flagged what must stay confidential.
- **Vibe copy review:** ten edits proposed, three adopted (full name of the library in EN, "environments", a two-sentence rewrite of the first Czech bullet); one was a false spelling finding on text that was already correct; six were tense or tone changes rejected because the entry is a current engagement and the summaries are noun phrases by convention. Cost: one call, under a minute.
- **Copilot test-gap review:** four gaps proposed, three adopted as tests (entry ids equal catalog keys in both locales, period format and order, every mark file exists in `public/`, every company name reaches the PDF); one gap was already covered by the accessibility spec, and its "inconsistent `roleId`" finding misread a data-only field. Its first shell call (`git diff` on named paths) was denied by its own permission model; the fallback `git diff --stat` plus file reads was enough. 1m41s, 4.22 credits.
- **Explore subagent:** produced the public facts (repo, licence, stack, Karel's public commit) and a confidentiality list (pricing, chat exports, a pending procurement, named individuals) that the copy then avoided. This is the packet shape to reuse when a client folder holds both public and private material.
- **Outcome:** review council cost about four minutes of wall-clock beside the pinned runs and changed three sentences and three tests; controller judgement rejected half of each reviewer's list, which is the expected ratio for this kind of packet.
