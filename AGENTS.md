...
- Approved copy (a sentence a decision record locks: brand headline, hero paragraphs, CTA labels, the availability line) is asserted **verbatim** in Playwright, never read back from `messages/*.json` — reading the catalog would only prove the page renders whatever the catalog holds. The approved strings live once, in `src/__tests__/e2e/support/approved-copy.ts`; specs import them, and `src/__tests__/unit/approved-copy.test.ts` keeps the fixture equal to both catalogs so drift fails in Vitest, not in the pinned container. Changing such a sentence means: catalog, fixture, decision record, and re-measuring any parity spec whose line counts depend on it. Extend the fixture (a new export per surface) when another route's copy gets locked; copy without a decision record stays in the catalog only.
