...
test('signed revalidation verifies freshness and body integrity', () => {
  ...
  assert.equal(verifySignedRevalidationRequest({ ...request, now: now + 1_000, secret })?.deliveryID, 'delivery-1')
  assert.equal(
    verifySignedRevalidationRequest({ ...request, now: now + 301_000, secret }),
    null,
  )
  assert.equal(
...
test('revalidation targets are derived from an allowlisted event, never accepted from clients', () => {
  ...
  assert.throws(
    () => getRevalidationTargets({ collection: 'posts', documentID: 42, operation: 'update', slug: '../admin' }),
