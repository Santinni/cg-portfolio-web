...
const isEvent = (value: unknown): value is RevalidationEvent => {
	...
	if (
		Object.keys(event).some(
			(key) => !['collection', 'documentID', 'operation', 'previousSlug', 'slug'].includes(key),
		)
	)
		return false
	if (!COLLECTIONS.includes(event.collection as RevalidationEvent['collection'])) return false
	if (
		!OPERATIONS.includes(event.operation as RevalidationEvent['operation']) ||
		!isID(event.documentID)
	)
		return false
...
const assertSecret = (secret: string): void => {
	if (secret.length < 32)
		throw new Error('Revalidation signing secret must contain at least 32 characters.')
}
...
export const verifySignedRevalidationRequest = ({
	...
	assertSecret(secret)
	if (body.length > 16_384 || !/^\d{13}$/u.test(timestamp) || !signature.startsWith('sha256='))
		return null
